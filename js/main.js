...
function renderAlerts() {
    ...
    function updateAlerts() {
        ...
        function computeRTWStats(files, rows) {
            ...
            // include common/short variants like 'End' and 'Next Duty' encountered in sample files
            ...
            // - `rows`: an array of matched row wrappers { data: <rowObj>, __locale: 'uk', __fileId, __fileName }
            // - `files`: an array of parsed file objects { dataRows: [...], _locale }
            const feedRows = [];
            if (Array.isArray(files) && files.length) {
                // build wrapper rows from each file's dataRows
                files.forEach(f => {
                    (f.dataRows || []).forEach(r => feedRows.push({ data: r, __locale: f._locale || 'uk', __fileId: f.id, __fileName: f.name }));
                });
            } else if (Array.isArray(rows) && rows.length) {
                // assume rows are already in wrapper form (as returned by buildResults)
                rows.forEach(r => feedRows.push(r));
            }
            ...
            feedRows.forEach(r => {
                ...
                for (const dc of dutyCols) {
                    ...
                    if (raw != null) {
                        // Determine shift type and priority. Accept 'combined' (prio=3), 'day' or 'night' (prio=2);
                        // if 'rest' is detected, skip adding this duty date. Use permissive fallback like diagnostics.
                        let dutyTypeVal = getValueByCanonical(r.data, 'Shift Type');
                        if (dutyTypeVal == null) {
                            // try some common alternate headers
                            const tryAlts = ['ShiftType','Type','Roster Type','Assignment Info','Assignment'];
                            for (const alt of tryAlts) {
                                const v = getValueByCanonical(r.data, alt);
                                if (v != null) { dutyTypeVal = v; break; }
                            }
                        }
                        let prio = -1;
                        ...
                        // permissive fallback: scan all fields for whole-word 'rest' if we couldn't determine prio
                        if (prio < 0) {
                            try {
                                ...
                                for (const key of Object.keys(r.data || {})) {
                                    try {
                                        const s = fieldToString(r.data[key]);
                                        if (!s) continue;
                        ...
                        if (prio < 0) {
                            if (window.__ALERTS_DEBUG) console.debug('computeRTWStats: skipping duty because Shift Type token not accepted', { row: r, file: r.__fileName, shiftType: dutyTypeVal });
                            break;
                        }
                        // If this is a Rest (prio === 0), skip adding it as a duty date entirely
                        if (prio === 0) {
                            if (window.__ALERTS_DEBUG) console.debug('computeRTWStats: detected Rest shift — skipping duty date', { row: r, file: r.__fileName });
                            break;
                        }
                        const dt = parseToDate(raw, r.__locale || 'uk');
                        if (dt) {
            ...
            Object.values(staffMap).forEach(s => {
                ...
                if (s.dutyDates.length) {
                    // group by iso and pick the entry with max prio for that date
                    const byIso = {};
                ...
                // nextDuty: earliest duty on/after today
                const nextDuty = processedDuties.find(d => {
                    const d0 = new Date(d.getFullYear(), d.getMonth(), d.getDate());
                    return d0.getTime() >= today.getTime();
                }) || null;
                // lastDuty: most recent duty on-or-before today (include today)
                ...
                // any duty after sickness? Use lastDuty (on-or-before today) so same-day counts as returned
